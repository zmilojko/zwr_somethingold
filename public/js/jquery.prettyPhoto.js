...
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = jQuery.extend({
			...
			opacity: 0.10, /* Value between 0 and 1 */
...
var pp_alreadyInitialized = false; // Used for the deep linking to make sure not to call the same function several times.
		...
		$(document).ready(function(){			
			//for prettyPhoto
			jQuery("a[data-gal^='prettyPhoto']").prettyPhoto({theme:'facebook'});
			$(function(){
	// lightbox image
			$(".lightbox-video").append("<span></span>");
			$(".lightbox-video").find('img').animate({opacity:.75},0)
			$(".lightbox-video").hover(function(){
				$(this).find('img').stop().animate({opacity:1})
			}, function(){
				$(this).find('img').stop().animate({opacity:.75})
			});
		});
		});
